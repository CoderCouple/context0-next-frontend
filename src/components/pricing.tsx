...
export function Pricing() {
  ...
  {tabs.map((tab) => (
    <button
      key={tab.value}
      className={cn(
        "text-sm font-medium text-gray-500 dark:text-neutral-400  p-4 rounded-md relative transition-colors duration-200",
        active === tab.value ? "text-white dark:text-white" : "text-neutral-900 dark:text-neutral-100",
      )}
      ...
      <div className="mx-auto mt-4 md:mt-20   grid relative z-20 grid-cols-1 gap-4 items-center  md:grid-cols-2 xl:grid-cols-4">
        {tiers.map((tier) => (
          <div
            ...
            className={cn(
              tier.featured
                ? "relative bg-[radial-gradient(164.75%_100%_at_50%_0%,#f97316_0%,#7c3aed_100%)] shadow-2xl"
